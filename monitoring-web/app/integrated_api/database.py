"""Azure SQL access for the integrated monitoring API."""
import logging
import os
...
def insert_metric(
    ...
    try:
        with get_connection() as conn:
            conn.execute(sql, (source_host, source_type, metric_name, metric_value, unit, status))
            conn.commit()
...
def get_metrics(
    ...
    sql = """
        SELECT id, timestamp, source_host, source_type, metric_name, metric_value, unit, status
